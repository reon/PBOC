...
import com.broadthinking.pboc3.service.IPBOCService;
 ...
 *
 */
public class PBOC extends Applet implements AppletEvent, IPBOCService {
	/**
	 * app version 1.0.0
	 */
	...
	private static final boolean FUNCTION_FOR_TIANYU	= false;
	...
	private PBOC() {
		...
		abyPBOCTradeSession = JCSystem.makeTransientByteArray(TRADE_SESSION_DATA_BUF_SIZE, JCSystem.CLEAR_ON_RESET);
		...
		signMac = Signature.getInstance(Signature.ALG_DES_MAC8_ISO9797_1_M2_ALG3, true);
		signMac.init(tripleDesKey, Signature.MODE_SIGN);
		cipherECBEncrypt = Cipher.getInstance(Cipher.ALG_DES_ECB_NOPAD, true);
		cipherECBEncrypt.init(tripleDesKey, Cipher.MODE_ENCRYPT);
		cipherECBDecrypt = Cipher.getInstance(Cipher.ALG_DES_ECB_NOPAD, true);
		cipherECBDecrypt.init(tripleDesKey, Cipher.MODE_DECRYPT);
		msgDigest = MessageDigest.getInstance(MessageDigest.ALG_SHA, true);
...
//		}
	...
	public void process(APDU apdu) throws ISOException {
		...
		if (selectingApplet()) {
			// var init
			Util.arrayFillNonAtomic(abyPBOCTradeSession, (short)0x00, TRADE_SESSION_DATA_BUF_SIZE, (byte)0x00);
			Util.arrayFillNonAtomic(abyCurTradeCardData, (short)0x00, CURRENT_TRADE_CARD_DATA_BUF_SIZE, (byte)0x00);
			curTradeConditions[0x00] = false;
			curTradeConditions[0x01] = false;
			curTradeConditions[0x02] = false;
			curTradeConditions[0x03] = false;
			curTradeConditions[0x04] = false;
	...
	public short transmitAPDU(byte[] inCommand, byte[] outCommand) {
		...
		try {
			...
			byte ins = inCommand[ISO7816.OFFSET_INS];
			switch (ins) {
			case CMD_INS_READ_RECORD:
				sLen = onReadRecord(inCommand, outCommand);
				break;
			case CMD_INS_GPO:
				sLen = onGPO(inCommand, outCommand);
				break;
			case CMD_INS_UPDATE_RECORD:
				sLen = onUpdateRecord(inCommand, outCommand);
				break;
			case CMD_INS_GENERATE_AC:
				sLen = onGenerateAC(inCommand, outCommand);
				break;
			case CMD_INS_SELECT:
				sLen = onSelect(inCommand, outCommand);
				break;
			case CMD_INS_GET_DATA:
				sLen = onGetData(inCommand, outCommand);
				break;
			case CMD_INS_PUT_DATA:
				sLen = onPutData(inCommand, outCommand);
				break;
			case CMD_INS_VERIFY_PIN:
				sLen = onVerifyPIN(inCommand, outCommand);
				break;
			case CMD_INS_CHANGE_UNBLOCK_PIN:
				sLen = onChangeUnBlockPIN(inCommand, outCommand);
				break;
			case CMD_INS_INTERN_AUTH:
				sLen = onInternalAuth(inCommand, outCommand);
				break;
			case CMD_INS_CARD_LOCK:
				sLen = onLockCard(inCommand, outCommand);
				break;
			case CMD_INS_APP_BLOCK:
				sLen = onAppOp(inCommand, outCommand);
				break;
			case CMD_INS_APP_UBLOCK:
				sLen = onAppOp(inCommand, outCommand);
				break;
			case CMD_INS_EXTERN_AUTH:
				sLen = onExternalAuth(inCommand, outCommand);
				break;
			case CMD_INS_STORE_DATA:
				//store data in INIT, append record in ISSUED
				if (appState == APP_STATE_ISSUED) {
					...
					sLen = onStoreData(inCommand, outCommand);
				}
				break;
			default:
				ISOException.throwIt(ISO7816.SW_INS_NOT_SUPPORTED);
			...
			sLen = Util.setShort(outCommand, (short)0x00, ex.getReason());
		} catch (Exception ex) {
			sLen = Util.setShort(outCommand, (short)0x00, (short)ISO7816.SW_UNKNOWN);
		}

		return sLen;
	}

	private short onReadRecord(byte[] apduBuf, byte[] outBuf) {
		// check class
		if ((byte)(apduBuf[ISO7816.OFFSET_CLA]&0xFC) != 0x00) {
			ISOException.throwIt(ISO7816.SW_CLA_NOT_SUPPORTED);
		}
		...
		if (sTmp != dgi) {
			// try read trade log file
			short sLen = readTradeLog(sfi, tradeLogFile, (short)(recNo&0x0FF), outBuf);

			// try read charge log file
			if (sLen == 0x00) {
				sLen = readChargeLog(sfi, (short)(recNo&0x0FF), outBuf);
			}

			// try read extend application trade log file
			if (sLen == 0x00) {
				sLen = readTradeLog(sfi, extendlogFile, (short)(recNo&0x0FF), outBuf);
			}
		...
		byte[] record = (byte[])recordObj[index];
		Util.arrayCopyNonAtomic(record, (short)0x00, outBuf, (short)0x00, (short)record.length);

		short sLen = outBuf[0x01];
		if (sLen == (byte)0x81) {
			sLen = (short)(outBuf[0x02]&0x0FF);
			sLen += 0x03;
		} else {
			sLen += 0x02;
		}

		return Util.setShort(outBuf, sLen, ISO7816.SW_NO_ERROR);
	}

	private short onUpdateRecord(byte[] apduBuf, byte[] outBuf) {
		// check class
		if ((byte)(apduBuf[ISO7816.OFFSET_CLA]&0xFC) != (byte)0x04) {
			ISOException.throwIt(ISO7816.SW_CLA_NOT_SUPPORTED);
		}
		...
		return Util.setShort(outBuf, (short)0x00, ISO7816.SW_NO_ERROR);
	}

	private short onSelect(byte[] apduBuf, byte[] outBuf) {
		if ((byte)(apduBuf[ISO7816.OFFSET_CLA]&0xFC) != 0x00) {
			ISOException.throwIt(ISO7816.SW_CLA_NOT_SUPPORTED);
		}
		...
		abyPBOCTradeSession[TRADE_SESSION_DATA_OFF_STATE] = TRADE_STATE_APP_SELECT;
		// set trade interface
		abyPBOCTradeSession[TRADE_SESSION_DATA_OFF_TRADE_INTERFACE] = TRADE_INTERFACE_CONTACT;

		short sLen = (short) contactfci.length;
		Util.arrayCopyNonAtomic(contactfci, (short)0x00, outBuf, (short)0x00, sLen);
		...
		if (appState == APP_STATE_LOCKED
			...
			return Util.setShort(outBuf, sLen, SW_SELECTED_FILE_DEACTIVED);
		}

		return Util.setShort(outBuf, sLen, ISO7816.SW_NO_ERROR);
	}

	private short onGetData(byte[] apduBuf, byte[] outBuf) {
		// check class
		if ((byte)(apduBuf[ISO7816.OFFSET_CLA]&0xFC) != (byte)0x80) {
			ISOException.throwIt(ISO7816.SW_CLA_NOT_SUPPORTED);
		}
		...
		// PIN Retry counter
		case TAG_PIN_RETRY_CNTR:
			sLen = PBOCUtil.appendTLV(sTag, paramBuf, PBOC_PARAM_OFF_PIN_LEFT_CNTR, (short)0x01, outBuf, (short)0x00);
			break;
		case TAG_AVAILABLE_OFFLINE_MONEY:
			// 可用脱机余额0x9F5D，在卡片附加处理“返回可用脱机余额”置位时才返回
//			if (isCPPReturnAvailableMoney) {
				calAvailableMoney();
				sLen = PBOCUtil.appendTLV(sTag, abyCurTradeCardData, getDataTags[index], (short)0x06, outBuf, (short)0x00);
//			} else {
//				ISOException.throwIt(SW_REFERENCED_DATA_NOT_FOUND);
//			}
			break;
		case TAG_LOG_FORMAT:
			if (logFormat == null) {
				Util.setShort(outBuf, (short)0x00, TAG_LOG_FORMAT);
				outBuf[0x02] = 0x00;
				sLen = 0x03;
			} else {
				sLen = PBOCUtil.appendTLV(sTag, logFormat, (short)0x00, (short)logFormat.length, outBuf, (short)0x00);
			}
			break;
		case TAG_CHARGE_LOG_FORMAT:
			if (chargelogFormat == null) {
				...
				sLen = PBOCUtil.appendTLV(sTag, chargelogFormat, (short)0x00, (short)chargelogFormat.length, outBuf, (short)0x00);
			}
			break;
		case TAG_CAPP_SECTION_PURCHASE_APP_ID:
			if (sExtAppIndicateOff != INVALID_VALUE) {
			...
			sLen = PBOCUtil.appendTLV(sTag, abyPBOCTradeSession, TRADE_SESSION_DATA_OFF_EXT_APP_INDICATE, (short)0x01, outBuf, (short)0x00);
			break;
		case TAG_GET_APP_VERSION:
			sLen = PBOCUtil.appendTLV(sTag, app_version, (short)0x00, (short)app_version.length, outBuf, (short)0x00);
			break;
		case TAG_GET_APP_NAME:
			sLen = PBOCUtil.appendTLV(sTag, app_name, (short)0x00, (short)app_name.length, outBuf, (short)0x00);
			break;
		default:
			short sOff = getDataTags[index++];
			short sValueLen = getDataTags[index];
			...
			sLen = PBOCUtil.appendTLV(sTag, cardDataBuf, sOff, sValueLen, outBuf, (short)0x00);
		}

		return Util.setShort(outBuf, sLen, ISO7816.SW_NO_ERROR);
	}

	private short onPutData(byte[] apduBuf, byte[] outBuf) {
		// check class
		if ((byte)(apduBuf[ISO7816.OFFSET_CLA]&0xFC) != (byte)0x04) {
			ISOException.throwIt(ISO7816.SW_CLA_NOT_SUPPORTED);
		}
	...
	private short onVerifyPIN(byte[] apduBuf, byte[] outBuf) {
		// check class
		if ((byte)(apduBuf[ISO7816.OFFSET_CLA]&0xFC) != (byte)0x00) {
			ISOException.throwIt(ISO7816.SW_CLA_NOT_SUPPORTED);
		}
	...
	private short onChangeUnBlockPIN(byte[] apduBuf, byte[] outBuf) {
		// check class
		if ((byte)(apduBuf[ISO7816.OFFSET_CLA]&0xFC) != (byte)0x84) {
			ISOException.throwIt(ISO7816.SW_CLA_NOT_SUPPORTED);
		}
	...
	private short onInternalAuth(byte[] apduBuf, byte[] outBuf) {
		// check class
		if ((byte)(apduBuf[ISO7816.OFFSET_CLA]&0xFC) != (byte)0x00) {
			ISOException.throwIt(ISO7816.SW_CLA_NOT_SUPPORTED);
		}
		...
		Util.arrayCopyNonAtomic(ddaTemplate, (short)0x00, outBuf, (short)0x00, sLen);
		msgDigest.doFinal(outBuf, (short)0x00, (short)0x00, outBuf, (short)(sLen-21));

		// replace ATC
		Util.arrayCopyNonAtomic(cardDataBuf, CARD_DATA_OFF_ATC, outBuf, DDA_OFF_IC_DATA_DIGIT, (short)0x02);

		cipherRSA.doFinal(outBuf, (short)0x00, sLen, outBuf, ISO7816.OFFSET_CDATA);

		sLen = PBOCUtil.appendTLV((short)0x80, outBuf, ISO7816.OFFSET_CDATA, sLen, outBuf, (short)0x00);

		return Util.setShort(outBuf, sLen, ISO7816.SW_NO_ERROR);
	}

	private short onLockCard(byte[] apduBuf, byte[] outBuf) {
		// check class
		if ((byte)(apduBuf[ISO7816.OFFSET_CLA]&0xFC) != (byte)0x84) {
			ISOException.throwIt(ISO7816.SW_CLA_NOT_SUPPORTED);
		}
	...
	private short onAppOp(byte[] apduBuf, byte[] outBuf) {
		// check class
		if ((byte)(apduBuf[ISO7816.OFFSET_CLA]&0xFC) != (byte)0x84) {
			ISOException.throwIt(ISO7816.SW_CLA_NOT_SUPPORTED);
		}
	...
	private short onExternalAuth(byte[] apduBuf, byte[] outBuf) {
		// check class
		if ((byte)(apduBuf[ISO7816.OFFSET_CLA]&0xFC) != (byte)0x00) {
			ISOException.throwIt(ISO7816.SW_CLA_NOT_SUPPORTED);
		}
	...
	private short onGPO(byte[] apduBuf, byte[] outBuf) {
		if ((byte)(apduBuf[ISO7816.OFFSET_CLA]&0xFC) != (byte)0x80) {
			ISOException.throwIt(ISO7816.SW_CLA_NOT_SUPPORTED);
		}
		...
		short sPDOLValueLen = (short)(paramBuf[PBOC_PARAM_OFF_PBOCPDOLVALUE_LEN]&0x0FF);
		...
		short sLen = PBOCGPOProcess(outBuf);

		return Util.setShort(outBuf, sLen, ISO7816.SW_NO_ERROR);
	}

	private short onStoreData(byte[] apduBuf, byte[] outBuf) {
		if (appState == APP_STATE_ISSUED) {
			ISOException.throwIt(ISO7816.SW_CONDITIONS_NOT_SATISFIED);
		}
		...
		if ((sfi > 0x00) && (sfi < 0x1F)) {
			...
			case DGI_PERSO_8000:
				...
				Util.arrayCopyNonAtomic(apduBuf, sOff, paramBuf, PBOC_PARAM_OFF_APP_KEY, (short) 0x30);
				break;
			// dCVN key
			case DGI_PERSO_8001:
				break;
			// offline PIN
			case DGI_PERSO_8010:
				Util.arrayCopyNonAtomic(apduBuf, sOff, paramBuf, PBOC_PARAM_OFF_PIN_VALUE, (short) 0x06);
				break;
			// RSA Key
			case DGI_PERSO_8201:
			case DGI_PERSO_8202:
			...
			case DGI_PERSO_9010:
				...
				Util.arrayCopyNonAtomic(apduBuf, sOff, paramBuf, PBOC_PARAM_OFF_PIN_LEFT_CNTR, (short) 0x02);
				break;
			// Select FCI(PBOC)
			case DGI_PERSO_9102:
			// Select FCI(qPBOC)
			...
			case DGI_PERSO_9200:
				...
				pbocIssueAppData = new byte[sIssueAppData];
				Util.arrayCopyNonAtomic(apduBuf, (short) (sOff+1), pbocIssueAppData, (short)0x00, (short)(apduBuf[sOff]&0x0FF));
				break;
			case DGI_PERSO_A001:
			case DGI_PERSO_8020:
			case DGI_PERSO_9020:
				persoExtendFile(null, dgi, apduBuf, sOff, sLen);
				break;
			default:
				ISOException.throwIt(SW_REFERENCED_DATA_NOT_FOUND);
			}
		...
		if ((byte)(p1&0x80) == (byte)0x80) {
			...
			appInit();
			buildGPORspTemplate(outBuf);
			buildDDATemplate(outBuf);
			createLogFile();
			bulidLogTemplate();
	...
	private short onGenerateAC(byte[] apduBuf, byte[] outBuf) {
		// check class
		if ((byte)(apduBuf[ISO7816.OFFSET_CLA]&0xFC) != (byte)0x80) {
			ISOException.throwIt(ISO7816.SW_CLA_NOT_SUPPORTED);
		}
		...
		// 根据货币转换因子转换近似授权金额
		if (!PBOCUtil.isAllZero(cardDataBuf, CARD_DATA_OFF_COIN_CONVERT_GENE, (short)0x04)) {
			Util.arrayFillNonAtomic(outBuf, (short)0x00, (short)33, (byte)0x00);

			Util.arrayCopyNonAtomic(abyCurTradeCDOLValue, tradeAuthMoneyOff, outBuf, (short)0x00, (short)0x06);
			Util.arrayCopyNonAtomic(cardDataBuf, CARD_DATA_OFF_COIN_CONVERT_GENE, outBuf, (short)0x08, (short)0x04);
			// 货币转换因子 4 byte
			// 字节1 位8-5：小数点位置。从右边开始移动的位数 位4-1：转换因子的第1个数字
			// 字节2-4：剩下的6个数字
			byte gene = (byte)(outBuf[0x08]>>0x04);
			outBuf[0x08] &= 0x0F;

			PBOCUtil.arrayDecMul(outBuf, (short)0x00, outBuf, (short)0x06, outBuf, (short)12, gene);

			Util.arrayCopyNonAtomic(outBuf, (short)12, abyPBOCTradeSession, TRADE_SESSION_DATA_OFF_TRADE_AUTH_NEAR_MONEY, (short)0x06);
		}
		...
		if (cmdCntr == 0x00) {
			// card risk manager
			cardTradeResult = cardRiskManager_1(acType, pbocpdol, outBuf);
			// gete response data
			sLen = getGenerateACRsp_1(outBuf, acType, cardTradeResult, bCDA);
		} else {
			// 第二条Generate AC指令请求ARQC返回0x6985
			if (acType == GENERATE_AC_TYPE_ARQC) {
				ISOException.throwIt(ISO7816.SW_CONDITIONS_NOT_SATISFIED);
			...
			if ((Util.arrayCompare(AUTH_CODE_Y3, (short)0x00, cdol2Value, authCodeOff, (short)0x02) == 0x00)
				 ...
				 */
				if (!PBOCUtil.isAllZero(cardDataBuf, CARD_DATA_OFF_TOTAL_CARD_MONEY_UPLIMIT, (short)0x06)) {
					PBOCUtil.arrayDecAdd(abyCurTradeCardData, CARD_DATA_OFF_TOTAL_OFFILINE_MONEY, cdol2Value, authMoneyOff, outBuf, (short)0x00);
					if (PBOCUtil.arrayCompare(outBuf, (short)0x00, cardDataBuf, CARD_DATA_OFF_TOTAL_CARD_MONEY_UPLIMIT, (short)0x06) == 1) {
						PBOCUtil.setBit(abyPBOCTradeSession, TRADE_SESSION_DATA_OFF_CVR, CVR_OFF_EXCEED_FREQ_CHECK);
						cardTradeResult = TRADE_RESULT_AAC;
					}
					 ...
					 */
					if (Util.arrayCompare(cardDataBuf, CARD_DATA_OFF_APPCOINCODE, cdol2Value, (short)(terDataInCDOLVOff[CDOLV_OFF_CDOL2_TRADE_COIN_CODE]&0x0FF), (short)0x02) == 0x00) {
						PBOCUtil.arrayDecAdd(abyCurTradeCardData, CARD_DATA_OFF_DCOINTOTAL_OFFILINE_MONEY, cdol2Value, authMoneyOff, outBuf, (short)0x00);
					} else {
						PBOCUtil.arrayDecAdd(abyCurTradeCardData, CARD_DATA_OFF_DCOINTOTAL_OFFILINE_MONEY, abyPBOCTradeSession, TRADE_SESSION_DATA_OFF_TRADE_AUTH_NEAR_MONEY, apduBuf, (short)0x00);
					}

					if (PBOCUtil.arrayCompare(outBuf, (short)0x00, cardDataBuf, CARD_DATA_OFF_TOTAL_CARD_MONEY_UPLIMIT, (short)0x06) == 0x01) {
						PBOCUtil.setBit(abyPBOCTradeSession, TRADE_SESSION_DATA_OFF_CVR, CVR_OFF_EXCEED_FREQ_CHECK);
						cardTradeResult = TRADE_RESULT_AAC;
					}
				}
				...
				sLen = getGenerateACRsp_2(outBuf, bCDA, false, acType, cardTradeResult);
			} else {
				// 授权响应码不是Y3或Z3表明是联机授权交易
				sLen = getGenerateACRsp_2(outBuf, bCDA, true, acType, cardTradeResult);
			}
		}
